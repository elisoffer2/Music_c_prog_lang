...
// refactor code
// fix very long song memory issues
...
// wave shaping and tone options of different instruments steady state wave forms
// transient sound on physical key pressing / string plucking
// 17. shuffle play
		...
		for (i = 0; i < signal_length; i++)
		{
			...
			for (k = 0; k < key_count; k++) {
				if (isSameNote && (i < tempoControl*SAME_NOTE_PAUSE_FACTOR)) {
					...
					// piano sound -1/4*sin(3*pi*x)+1/4*sin(pi*x)+(sqrt(3)/2)*cos(pi*x)
					note_signal[k][i] = (32767 / key_count) * (-1*0.25*sin((3.0*PI*notefreq[k] * i) / SAMP_RATE) + 0.25*sin((PI*notefreq[k] * i) / SAMP_RATE) +  0.866*cos((PI*notefreq[k] * i) / SAMP_RATE));
				}
